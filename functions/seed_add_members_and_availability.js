...
async function addAllTestUsersToBlades() {
  ...
  if (!teamDoc.exists) {
    console.error(
      `  ✗ Team ${HOCKEY_TEAM_ID} not found — run seed_test_users.js first.`,
    );
    process.exit(1);
  }
  ...
  const toAdd = testUsers.filter(
    (u) => !adminSet.has(u.uid) && !playerSet.has(u.uid),
  );
  ...
  const freshTeam = (
    await db.collection("teams").doc(HOCKEY_TEAM_ID).get()
  ).data();
...
async function seedAvailability(teamId, admins, players, yesCount) {
  ...
  if (eventsSnap.empty) {
    console.log(
      `  ⚠️  No upcoming events for ${teamId} — create events in the app first.\n`,
    );
    return;
  }
  ...
  console.log(`\n${"━".repeat(60)}`);
  console.log(
    `  ${teamName}  (${allMembers.length} members — ${yesUids.length} yes / ${noUids.size} no)`,
  );
  console.log("━".repeat(60));
  console.log(`  ✅ Yes: ${yesUids.map((u) => nameMap[u] ?? u).join(", ")}`);
  console.log(
    `  ❌ No:  ${[...noUids].map((u) => nameMap[u] ?? u).join(", ")}\n`,
  );
  ...
  for (const eventDoc of eventsSnap.docs) {
    ...
    await batch.commit();
    console.log(
      `       ✅ ${yesUids.length} yes  ❌ ${noUids.size} no  — written`,
    );
  }
}
...
async function main() {
  ...
  if (!dragonTeamDoc.exists) {
    ...
    await seedAvailability(
      ...
      dt.admins ?? [],
      dt.players ?? [],
      (dt.admins?.length ?? 0) + (dt.players?.length ?? 0) - 2,
    );
  }
