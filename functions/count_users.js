...
async function countUsers() {
  ...
  const real = all.filter((u) => {
    if (u.deleted === true) {
      excluded.push(`${u.name} <${u.email}> [deleted]`);
      return false;
    }
    if (u.role === "systemAdmin") {
      excluded.push(`${u.name} <${u.email}> [systemAdmin]`);
      return false;
    }
    if ((u.name || "").toLowerCase().includes("test")) {
      excluded.push(`${u.name} <${u.email}> [test name]`);
      return false;
    }
    if ((u.email || "").toLowerCase().includes("test")) {
      excluded.push(`${u.name} <${u.email}> [test email]`);
      return false;
    }
    return true;
  });
  ...
  const appleRelays = real.filter((u) =>
  ...
  appleRelays.forEach((u) => {
    ...
    console.log(
      `Created:   ${u.createdAt?.toDate ? u.createdAt.toDate().toISOString() : u.createdAt}`,
    );
