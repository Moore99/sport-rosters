...
class _BoatSeatingScreenState extends ConsumerState<BoatSeatingScreen> {
  ...
  Future<void> _balance(List<String> availableUids, BoatConfig cfg) async {
    ...
    for (int i = 0; i < sorted.length; i++) {
      ...
      final boatIdx =
          dir == 1 ? indexInRound : (cfg.numBoats - 1 - indexInRound);
  ...
  Future<void> _exportPdf(BuildContext context, WidgetRef ref, Team team,
            ...
            return lineupAsync.when(
              ...
              data: (saved) {
                ...
                final availableUids = ref
                        .watch(eventAvailabilityProvider(
                            (widget.eventId, widget.teamId)))
                        .valueOrNull
                        ?.where((a) =>
                            a.response.name == 'yes' ||
                            a.response.name == 'maybe')
                        .map((a) => a.userId)
                        .where((u) =>
                            team.players.contains(u) || team.admins.contains(u))
                        .toList() ??
                    [];
                ...
                return Scaffold(
                  appBar: AppBar(
                    ...
                    actions: [
                      if (isAdmin) ...[
                        TextButton(
                            onPressed: _clearAll, child: const Text('Clear')),
                        _balancing
                            ? const Padding(
...
class _BoatCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    for (int r = 1; r <= cfg.rowsPerBoat; r++) {
      final lw = ref
              .watch(_weightProvider(draft['Boat $boatNum Row $r Left'] ?? ''))
              .valueOrNull ??
          0.0;
      final rw = ref
              .watch(_weightProvider(draft['Boat $boatNum Row $r Right'] ?? ''))
              .valueOrNull ??
          0.0;
      leftKg += lw;
      rightKg += rw;
  ...
  Color _frontBackColor(
      double thisSide, double otherSide, BuildContext context) {
...
class _SeatTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final profileAsync =
        uid.isNotEmpty ? ref.watch(_profileProvider(uid)) : null;
          ...
          children: [
            ...
            Text(
              ...
              style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                    color:
                        assigned ? null : Theme.of(context).colorScheme.outline,
            ...
            child: ListView(
              ...
              children: [
                ...
                if (placed.isNotEmpty) ...[
                  ...
                  for (final uid in placed)
                    memberTile(uid, placedAt: uidToPos[uid]),
                ],
              ],
            ),
          ),
