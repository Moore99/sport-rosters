...
| Ads | Google AdMob (mobile), AdSense stub (web — activate when approved) |
| IAP | `in_app_purchase` plugin + server-side validation (mobile); Stripe Checkout (web) |
| Web hosting | Firebase Hosting (sports-rostering.web.app) |

## Route Structure
```
/                                 ← marketing landing page (unauthenticated web entry; authenticated → /teams)
...
- Unauthenticated users on web at `/` → `/` (landing page shown)
- Unauthenticated users elsewhere → `/login`
...
## Future / Deferred
- **AdSense banner ads** — `banner_ad_widget.dart` returns `SizedBox.shrink()` on web pending AdSense approval. Uncomment script in `web/index.html` and update `AppConfig.adsensePublisherId` / `AppConfig.adsenseBannerAdUnitWeb` when approved.
- **Stripe prerequisites** — `createStripeCheckout` Cloud Function has `STRIPE_PRICE_ID = 'price_TODO_REPLACE'`. Set real price ID, then `firebase functions:secrets:set STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET`, and register the webhook URL in Stripe Dashboard.
- **GA4 Measurement ID** — add `measurementId: 'G-XXXXXXXXXX'` to the web block in `lib/firebase_options.dart` after creating a GA4 property in Firebase Console.
- **Apple Sign-In web** — Services ID `com.sportsrostering.app.signin` must have `sports-rostering.web.app` as domain and `https://sports-rostering.web.app/__/auth/handler` as return URL in Apple Developer portal.
