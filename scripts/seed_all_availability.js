...
async function seedTeamEvents(teamId, label, noCount) {
  ...
  if (events.length === 0) {
    console.log(
      "  ⚠️  No upcoming events found — create events in the app first.\n",
    );
    return;
  }
  ...
  for (const event of events) {
    ...
    console.log(
      `  📅  ${event.type.toUpperCase()}  ${eventDate}  (${event.id})`,
    );
    ...
    console.log(
      `       ✅ Yes (${yesUids.length}): ${yesUids.map((u) => nameMap[u] ?? u).join(", ")}`,
    );
    console.log(
      `       ❌ No  (${noUids.size}): ${[...noUids].map((u) => nameMap[u] ?? u).join(", ")}`,
    );
  }
}
