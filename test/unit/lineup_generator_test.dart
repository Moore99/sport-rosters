...
PlayerPreference _pref(String uid, List<String> positions) => PlayerPreference(
    userId: uid, teamId: 't', preferredPositions: positions, updatedAt: _now);

const _hockeyPositions = [
  'Goalie',
  'Left Wing',
  'Centre',
  'Right Wing',
...
// ── generateSubTeams() ───────────────────────────────────────────────────────
  ...
  group('LineupGenerator.generateSubTeams', () {
    ...
    test('no player appears in two sub-teams', () {
      ...
      final all =
          result.expand((m) => m.values).where((v) => v.isNotEmpty).toList();
