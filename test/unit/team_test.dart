...
void main() {
  ...
  group('Team.isAdmin', () {
    ...
    test('false for player uid',
        () => expect(_make().isAdmin('player1'), isFalse));
    test('false for unknown uid',
        () => expect(_make().isAdmin('nobody'), isFalse));
  });
  ...
  group('Team.isMember', () {
    ...
    test(
        'false for unknown', () => expect(_make().isMember('nobody'), isFalse));
  });

  group('Team.totalMembers', () {
    test('counts admins + players',
        () => expect(_make().totalMembers, equals(3)));
    test('handles empty',
        () => expect(_make(admins: [], players: []).totalMembers, equals(0)));
    test(
        'admin-only', () => expect(_make(players: []).totalMembers, equals(1)));
  });
